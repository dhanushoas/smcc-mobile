  ...
  Future<void> _handleUpdate(String type, dynamic value, [Map<String, dynamic>? params]) async {
    ...
    if (['runs', 'extra', 'wicket', 'retire', 'new_batsman', 'new_bowler'].contains(type)) {
       try {
         ...
         history.add(jsonDecode(jsonEncode(match)));
         // Limit history to 20 items to prevent bloat
  ...
  void _showNewBowlerDialog({bool isRetirement = false}) {
      ...
      showDialog(context: context, builder: (context) {
         return StatefulBuilder(builder: (context, setState) {
            return AlertDialog(
               shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
               title: Text(isRetirement ? '🚑 Replace Bowler' : '⚾ New Bowler', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.teal)),
               content: Column(
                 mainAxisSize: MainAxisSize.min,
                 children: [
                   Text(
           ...
           DropdownButtonHideUnderline(
             child: DropdownButton<String>(
               ...
               items: items.map<DropdownMenuItem<String>>((v) => DropdownMenuItem(
                 value: v.toString(), 
                 child: Text(_toCamelCase(v.toString()), style: TextStyle(fontWeight: FontWeight.bold))
               )).toList(),
